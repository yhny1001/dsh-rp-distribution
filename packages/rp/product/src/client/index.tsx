...
function ConversationContextButton({ sessionId }: { readonly sessionId: string }): ReactNode {
  ...
  return <button type="button" className={`rpp-header-context ${response?.binding === undefined ? 'rpp-header-context-empty' : ''}`}
    onClick={() => openProduct('compose', sessionId)} title={preset === undefined ? '编辑 RP 会话编排' : `System Prompt 已注入：${preset.name}`}>
    <span className="rpp-stack-icon"><i /><i /><i /></span>
    <span>{character === undefined ? '配置 RP' : `System ✓ · ${preset?.name ?? '预设'} · ${character.name}`}</span>
  </button>
}
...
function ConversationContextDock({ sessionId }: { readonly sessionId: string }): ReactNode {
  ...
  const extra = [persona?.name, world?.name, binding.scene.trim() || undefined].filter(value => value !== undefined)
  const details = [binding.mode === 'agent' ? 'Agent RP' : 'Tavern Chat', character?.name, preset?.name, ...extra].filter(value => value !== undefined).join(' · ')
  return <button type="button" className="rpp-context-dock" onClick={() => openProduct('compose', sessionId)}
    title={`当前 RP 配置：${details}。Prompt Preset 已通过 @deepseek-ai/dsh-system-prompt 注入 request.system；点击查看或更改。`}>
    <span className="rpp-context-selected">SYSTEM ✓</span>
    <AvatarFace character={character} className="rpp-context-avatar" fallback={character?.name.slice(0, 1) ?? '角'} />
    <span className="rpp-context-copy"><b>{character?.name ?? '未选择角色'}</b><small>
      {binding.mode === 'agent' ? 'Agent RP' : 'Tavern Chat'}{preset === undefined ? '' : ` · ${preset.name}`}
    </small></span>
    {extra.length === 0 ? null : <span className="rpp-context-extra">+{extra.length} 项</span>}
    <span className="rpp-context-edit">更改⌄</span>
  </button>
}

function SystemInjectionRoute({ presetName, compact = false }: { readonly presetName: string; readonly compact?: boolean }): ReactNode {
  return <div className={`rpp-system-route ${compact ? 'rpp-system-route-compact' : ''}`} aria-label="System Prompt 注入路径">
    <span className="rpp-system-route-badge">SYSTEM</span>
    <span className="rpp-system-route-flow"><b>{presetName || 'Prompt Preset'}</b><i>→</i><b>@deepseek-ai/dsh-system-prompt</b><i>→</i><b>request.system</b></span>
    <small>角色卡开场白与聊天记录留在原生 Session History；运行权限 Snapshot 不代表完整 System Prompt。</small>
  </div>
}
...
function QuickSetup({ response, sessionId, mode }: {
  ...
  return <section className="rpp-quick-setup" data-rp-quick-setup={mode}>
    ...
    <SystemInjectionRoute presetName={preset?.name ?? 'Prompt Preset'} compact />
    <details className="rpp-quick-customize"><summary>更改自动选择与开局场景</summary><div className="rpp-quick-grid">
      ...
      {mode === 'agent' ? <label>Experience<select value={draft.experienceId} onChange={event => setDraft(withExperience(draft, event.target.value, response.state))}>
  ...
  return <div className="rpp-compose-grid">
    <section className="rpp-compose-controls">
      ...
      {draft.mode === 'agent' ? <Field label="Agent Experience" note="决定自动维护和多 Agent 深度；不替换 Prompt Preset" accent="#e7a84f">
        <Select value={draft.experienceId} onChange={experienceId => setDraft(withExperience(draft, experienceId, response.state))} items={[
      ...
      {draft.mode === 'agent' && draft.experienceId === 'rp-multi-character' ? <div className={`rpp-cast-hint ${draft.characterIds.length < 2 ? 'rpp-cast-hint-warn' : ''}`}>
        <b>{draft.characterIds.length < 2 ? '还需要至少一位角色' : `已组成 ${String(draft.characterIds.length)} 人阵容`}</b>
        <span>{draft.characterIds.map(id => response.state.characters.find(character => character.id === id)?.name ?? id).join(' → ') || '请在下面勾选参与角色'}</span>
      </div> : null}
      ...
      <div className="rpp-compose-actions"><span>应用后写入 @deepseek-ai/dsh-system-prompt，并由原生 AgentLoop 继续对话</span><div className="rpp-compose-buttons">
    ...
    <aside className="rpp-preview">
      <div className="rpp-preview-heading"><span>System Prompt 注入预览</span><small>最终 request.system 顺序</small></div>
      <SystemInjectionRoute presetName={response.state.presets.find(item => item.id === draft.presetId)?.name ?? 'Prompt Preset'} compact />
      ...
      <div className="rpp-preview-note"><b>System 与 History 分层</b><span>下列非 History 层通过 System Prompt Registry 组成 request.system；聊天记录 Marker 与角色卡 first_mes 仍由原生 Session History 承载。</span></div>
    </aside>
  </div>
}
...
function PresetManager({ response }: { readonly response: ProductResponse }): ReactNode {
  ...
  return <div className="rpp-preset-layout">
    ...
    <section className="rpp-preset-main">
      ...
      <SystemInjectionRoute presetName={draft.name} compact />
      ...
      <footer className="rpp-preset-actions"><span>Prompt 的启停和顺序会通过 @deepseek-ai/dsh-system-prompt 直接改变下一次 request.system。</span>
...
function RpConversationView({ sessionId, useSession }: { readonly sessionId: string; readonly useSession: UseSession }): ReactNode {
  ...
  return <section className="rpp-story" data-rp-story={sessionId}>
    <header className="rpp-story-header"><div className="rpp-story-identity">
      ...
      <button type="button" disabled={client.saving || running} onClick={() => chatInput.current?.click()}>导入聊天</button>
      <button type="button" disabled={messages.length === 0} onClick={() => exportTavernChat(messages, character?.name ?? '角色', persona?.name ?? 'User')}>导出聊天</button>
      <button type="button" onClick={() => openProduct('compose', sessionId)}>设定</button>
      <input ref={chatInput} hidden type="file" accept=".jsonl,.json,application/json,text/plain" onChange={event => {
        const file = event.target.files?.[0]
        event.target.value = ''
        if (file !== undefined) void importTavernChat(file, sessionId, character?.name ?? '角色', persona?.name ?? 'User')
      }} />
    </div></header>
      ...
      <div className="rpp-story-thread">{messages.length === 0 ? <div className="rpp-story-empty rpp-story-empty-compact"><span>☾</span><h3>故事还没有开始</h3>
        ...
        {messages.map((message, index) => {
          ...
          <div className="rpp-message-head"><AvatarFace character={speakerCharacter} className="rpp-message-avatar" fallback={message.speakerName.slice(0, 1)} accent={message.role === 'assistant' ? speakerCharacter?.accent ?? '#f47f6b' : '#36b8d4'} />
            ...
            {message.role === 'assistant' ? <button type="button" title="使用当前浏览器的本地语音朗读，不上传音频" onClick={() => speakCharacterMessage(message.content, message.speakerName)}>朗读</button> : null}
...
function RuntimeProjection({ runtime, sessionId, running, characters }: {
  readonly runtime: SessionRuntimeState
  readonly sessionId: string
  readonly running: boolean
  readonly characters: readonly CharacterProfile[]
}): ReactNode {
  const projected = currentRuntimeEffects(runtime)
  const current = projected.filter(effect => effect.kind !== 'media')
  const media = projected.flatMap(effect => effect.kind === 'media' ? runtimeMediaArtifact(effect) : []).slice(-8)
  return <section className="rpp-runtime-projection">
    {media.length === 0 ? null : <section className="rpp-media-gallery"><header><span><small>SCENE MEDIA</small><b>本轮媒体</b></span><i>{media.length} 项</i></header>
      <div>{media.map(artifact => <article key={artifact.id}><div><small>{artifact.kind === 'image' ? 'IMAGE' : 'AUDIO'}</small><b>{artifact.title}</b><span>{artifact.mimeType}</span></div>
        {artifact.uri === '' ? <p>Artifact URI 需要对应 Provider UI 解析</p> : artifact.kind === 'image'
          ? <img src={artifact.uri} alt={artifact.title} />
          : <audio controls preload="metadata" src={artifact.uri}>当前浏览器不能播放此音频。</audio>}</article>)}</div></section>}
    {runtime.castQueue.length === 0 && runtime.lastSpeakerId === '' ? null : <div className="rpp-cast-queue"><header><span><small>GROUP TURN</small><b>角色发言队列</b></span><i>ROUND {runtime.castRound}</i></header>
      <div>{runtime.lastSpeakerId === '' ? null : <span className="rpp-cast-last">刚刚 · {characters.find(character => character.id === runtime.lastSpeakerId)?.name ?? runtime.lastSpeakerId}</span>}
        {runtime.castQueue.map((id, index) => {
          const character = characters.find(item => item.id === id)
          return <span key={`${id}:${index}`} className={index === 0 ? 'rpp-cast-next' : ''}><AvatarFace character={character} className="rpp-cast-avatar" fallback={character?.name.slice(0, 1) ?? '?'} />
            <b>{index === 0 ? '下一位 · ' : ''}{character?.name ?? id}</b></span>
        })}{runtime.castQueue.length === 0 ? <span className="rpp-cast-empty">本轮队列已完成</span> : null}</div></div>}
...
function effectIcon(kind: string): string {
  return kind === 'world' ? '界' : kind === 'time' ? '时' : kind === 'scene' ? '景' : kind === 'character' ? '角'
    : kind === 'persona' ? '我' : kind === 'npc' ? '众' : kind === 'relationship' ? '缘' : kind === 'memory' ? '忆'
      : kind === 'objective' ? '标' : kind === 'inventory' ? '物' : '媒'
}
...
function effectKindLabel(kind: string): string {
  return kind === 'world' ? '世界' : kind === 'time' ? '时间' : kind === 'scene' ? '场景' : kind === 'character' ? '角色'
    : kind === 'persona' ? 'Persona' : kind === 'npc' ? 'NPC' : kind === 'relationship' ? '关系' : kind === 'memory' ? '记忆'
      : kind === 'objective' ? '目标' : kind === 'inventory' ? '物品' : '媒体'
}

interface RuntimeMediaArtifact {
  readonly id: string
...
function runtimeMediaArtifact(effect: SessionRuntimeState['effects'][number]): RuntimeMediaArtifact[] {
  const value = effect.data.artifact
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return []
  const artifact = value as Record<string, unknown>
  if (typeof artifact.id !== 'string' || typeof artifact.mimeType !== 'string' || artifact.kind !== 'image' && artifact.kind !== 'audio' || typeof artifact.uri !== 'string') return []
  const uri = /^(?:data:|https:\/\/)/u.test(artifact.uri) ? artifact.uri : ''
  return [{ id: artifact.id, kind: artifact.kind, mimeType: artifact.mimeType, uri, title: effect.title }]
}
...
async function importTavernChat(file: File, sessionId: string, fallbackCharacter: string, fallbackPersona: string): Promise<void> {
  update({ saving: true, error: '', notice: '' })
  try {
    if (file.size === 0 || file.size > 4 * 1024 * 1024) throw new Error('聊天历史文件必须在 1 B–4 MiB 之间')
    const parsed = parseTavernChat(await file.text(), fallbackCharacter, fallbackPersona)
    const receipt = await sendCommand(sessionId, 'rp-studio-chat-import', { sessionId, messages: parsed.messages })
    update({ saving: false, notice: `${receipt}${parsed.skipped === 0 ? '' : `；跳过 ${String(parsed.skipped)} 条元数据或系统记录`}` })
    await loadProduct(sessionId)
  } catch (error: unknown) { update({ saving: false, error: publicError(error) }) }
}

function exportTavernChat(messages: readonly StoryMessage[], characterName: string, personaName: string): void {
  const blob = new Blob([serializeTavernChat(messages, characterName, personaName)], { type: 'application/x-ndjson;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const anchor = document.createElement('a')
  anchor.href = url
  anchor.download = `${slug(characterName)}-${Date.now()}.jsonl`
  anchor.click()
  URL.revokeObjectURL(url)
}

function speakCharacterMessage(content: string, speakerName: string): void {
  if (!('speechSynthesis' in window) || !('SpeechSynthesisUtterance' in window)) {
    update({ error: '当前浏览器没有本地语音朗读能力；可安装 Audio Media Provider 后使用 rp_generate_media。' })
    return
  }
  ...
  utterance.onerror = event => {
    if (event.error !== 'canceled' && event.error !== 'interrupted') update({ error: `${speakerName}的本地语音朗读失败：${event.error}` })
  }
...
function withExperience(draft: CompositionDraft, experienceId: string, state: ProductState): CompositionDraft {
  if (experienceId !== 'rp-multi-character') return { ...draft, experienceId }
  const imported = state.characters.filter(character => character.source !== undefined)
  const candidates = imported.length >= 2 ? imported : state.characters
  const characterIds = [...new Set([...draft.characterIds, ...candidates.map(character => character.id)])].slice(0, 16)
  return {
    ...draft,
    experienceId,
    characterIds,
    primaryCharacterId: characterIds.includes(draft.primaryCharacterId) ? draft.primaryCharacterId : characterIds[0] ?? '',
  }
}
...
.rpp-avatar-face{overflow:hidden}.rpp-avatar-face img{display:block;width:100%;height:100%;object-fit:cover;object-position:50% 18%}
...
.rpp-cast-hint{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:-2px 0 8px;padding:9px 10px;border:1px solid rgba(91,184,167,.24);border-radius:10px;color:#b8f3e7;background:rgba(91,184,167,.075);font-size:8px}.rpp-cast-hint span{overflow:hidden;color:var(--rpp-muted);text-overflow:ellipsis;white-space:nowrap}.rpp-cast-hint-warn{border-color:rgba(231,168,79,.3);color:#f1cf9a;background:rgba(231,168,79,.075)}
...
.rpp-system-route{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:5px 9px;margin:0 0 12px;padding:10px 11px;border:1px solid rgba(139,124,246,.2);border-radius:12px;background:linear-gradient(100deg,rgba(139,124,246,.09),rgba(78,198,164,.045))}.rpp-system-route-badge{padding:4px 6px;border-radius:6px;color:#d8d3ff;background:rgba(139,124,246,.18);font-size:7px;font-weight:800;letter-spacing:.09em}.rpp-system-route-flow{display:flex;align-items:center;gap:6px;min-width:0;color:var(--rpp-text);font-size:8px}.rpp-system-route-flow b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.rpp-system-route-flow b:first-child{color:#c7c0ff}.rpp-system-route-flow b:last-child{color:#77d6b9}.rpp-system-route-flow i{flex:none;color:var(--rpp-muted);font-style:normal}.rpp-system-route small{grid-column:2;color:var(--rpp-muted);font-size:7px;line-height:1.45}.rpp-system-route-compact{margin-block:9px 12px;padding-block:8px}
...
.rpp-quick-setup .rpp-system-route{border-color:color-mix(in srgb,#8b7cf6 24%,var(--dsw-alias-border-l2,rgba(127,127,127,.18)));background:linear-gradient(100deg,rgba(139,124,246,.075),rgba(85,216,154,.055))}.rpp-quick-setup .rpp-system-route-badge{color:#6e62ca;background:rgba(139,124,246,.13)}.rpp-quick-setup .rpp-system-route-flow{color:var(--dsw-alias-label-primary,#222733)}.rpp-quick-setup .rpp-system-route-flow b:first-child{color:#6e62ca}.rpp-quick-setup .rpp-system-route-flow b:last-child{color:#37845e}.rpp-quick-setup .rpp-system-route small{color:var(--dsw-alias-label-tertiary,#737b8a)}
...
.rpp-media-gallery{overflow:hidden;border:1px solid rgba(124,167,246,.25);border-radius:16px;background:linear-gradient(145deg,rgba(124,167,246,.07),rgba(139,124,246,.055))}.rpp-media-gallery>header{display:flex;align-items:center;justify-content:space-between;padding:11px 13px;border-bottom:1px solid rgba(124,167,246,.14)}.rpp-media-gallery>header>span{display:flex;flex-direction:column}.rpp-media-gallery>header small{color:#6f91d0;font-size:7px;font-weight:800;letter-spacing:.12em}.rpp-media-gallery>header b{font-size:11px}.rpp-media-gallery>header i{color:#6f91d0;font-size:7px;font-style:normal}.rpp-media-gallery>div{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:1px;background:rgba(124,167,246,.12)}.rpp-media-gallery article{display:flex;min-width:0;flex-direction:column;gap:8px;padding:10px;background:var(--dsw-alias-bg-raised,#fff)}.rpp-media-gallery article>div{display:flex;flex-direction:column}.rpp-media-gallery article small{color:#6f91d0;font-size:7px;font-weight:750}.rpp-media-gallery article b{font-size:10px}.rpp-media-gallery article span,.rpp-media-gallery article p{margin:1px 0 0;color:var(--dsw-alias-label-tertiary,#8f98aa);font-size:7px}.rpp-media-gallery img{display:block;width:100%;max-height:360px;border-radius:10px;background:#121722;object-fit:contain}.rpp-media-gallery audio{width:100%;height:34px}.rpp-effect-media{--effect:#7ca7f6}
.rpp-cast-queue{overflow:hidden;border:1px solid rgba(91,184,167,.25);border-radius:14px;background:linear-gradient(135deg,rgba(91,184,167,.08),rgba(54,184,212,.055))}.rpp-cast-queue>header{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid rgba(91,184,167,.15)}.rpp-cast-queue>header>span{display:flex;flex-direction:column}.rpp-cast-queue>header small{color:#4d9f91;font-size:7px;font-weight:800;letter-spacing:.12em}.rpp-cast-queue>header b{font-size:11px}.rpp-cast-queue>header i{color:#4d9f91;font-size:7px;font-style:normal}.rpp-cast-queue>div{display:flex;align-items:center;flex-wrap:wrap;gap:7px;padding:10px 12px}.rpp-cast-queue>div>span{display:flex;align-items:center;gap:5px;padding:4px 7px;border-radius:8px;color:var(--dsw-alias-label-secondary,#7d8492);background:var(--dsw-alias-bg-raised,#fff);font-size:8px}.rpp-cast-queue .rpp-cast-next{color:#378c7c;box-shadow:inset 0 0 0 1px rgba(91,184,167,.3)}.rpp-cast-avatar{display:grid;place-items:center;width:22px;height:22px;border-radius:7px;color:white;background:var(--rpp-accent);font-size:8px}.rpp-cast-last{opacity:.68}.rpp-cast-empty{font-style:italic}
