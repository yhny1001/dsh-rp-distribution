...
Each Experience has an execution policy. World Simulation emphasizes time, NPC, and objective progression. Multi-character uses `rp_schedule_cast` plus `rp_next_speaker` for a cross-Turn queue and retains `rp_select_speaker` as a manual override. TRPG `rp_roll` validates and executes `NdM±K` with at most 20 dice and 1,000 sides; its result is logged as a native Tool Result. Companion emphasizes relationship, memory, persona, and character continuity without requiring choices every round. These remain Prompt policies and plugin Tools inside the same `rp-agent`, not another AgentLoop.

State Keeper uses existing event extensions without changing AgentLoop. `agent/inbox/claimed` captures the Runtime revision for a Turn's first input. `agent/turn-stopping` compares the current revision and, when unchanged, steers an `@dsh-rp/product` `instructions` message requiring `rp_commit_turn`. An unchanged Turn may commit `updates: []`, which still advances Revision. Repair is attempted twice; a third unchanged stop throws and fails the Turn closed. Success, failure, and missing-baseline paths clear per-Turn tracking, and internal steering is not projected as character prose.
...
A real unchanged Kaon round asks for only “good night” with no choices. The model first omits the commit and emits “晚安。”. Turn-stop State Keeper steers a repair; the second step commits `updates: []` and advances Ledger from `REV 1` to `REV 2`. RP Conversation projects only the first non-empty Assistant prose per Turn, so internal audit steering, the repair acknowledgement, and reasoning remain hidden while the user sees only “晚安。”.
