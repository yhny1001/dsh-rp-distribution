...
`@dsh-rp/product` 是独立于完整发行族的本地优先产品 Bundle。它把 Node API、Client UI、Clean-room SillyTavern 导入路径和独立的 `rp-tavern`/`rp-agent` Agent Preset 打进一个可安装 Tarball，只使用 DSH `0.1.0-rc.6` 已存在的 `webServer`、`commands`、`agentPresets`、Session Event 与正式 Web Slot。它不启动第二套生成循环；二百五十六个稳定 System Prompt Seat 从顺序 1000 开始投影可变的 Prompt Manager 顺序，最多一千零二十四个保留定义容纳未编排备选。Host 基础段与 ST Role Protocol 位于导入 Seat 之前；以 Assistant Prefill 结尾的顺序把该层保留在最后一个 Seat，产品 Runtime Mode 紧邻其前，使 DSH 工具要求不覆盖 ST 的最终生成起点。带未闭合 planning/thinking/reasoning 标记的 Prefill 保留为转义源文本并声明私有 Reasoning 意图；模型仍泄漏到普通 Text Block 时，纯展示投影确定性移除具名私有规划区块，同时保留完整 Session 原文。全部非 History 层通过 `@deepseek-ai/dsh-system-prompt` 进入 `request/header.header.system`；Client 显示到 Provider `request.system` 的明确路径，并把 Host 同名动态 Context Snapshot 标注为不同载体，避免用户误把权限快照当成完整 System Prompt。Marker 分别解析系统规则、世界观、角色阵容、用户 Persona、场景、示例与 Host 所属的聊天历史位置；纯文本 Assembler 解析身份宏与按启用顺序生效的 `setvar/getvar`，不执行脚本。`agent/inbox/claimed` 提供首步尚未进入 Session Surface 的当前用户正文，保证 `lastUserMessage` 在第一次模型请求中即可展开；后续 Step 读取最新原生用户消息。导入源文档挂在可直接选择的 `sillytavern` Preset 上；经过用户明确确认的非破坏式适配会创建或刷新独立 `harness` Preset，把注入元数据归一为顺序 Seat，同时保留来源、正文、启用状态与生成参数。受支持的生成参数通过 `agent/request` Waterfall 进入请求，并由 Request Header 记录。按内容哈希寻址的 PNG 角色卡字节与产品状态并列存储，并通过同源不可变 Asset Route 提供。产品 Transcript Projection 在每条 append-origin Message 落地时记录当时的 Persona 或主要角色；正文编辑保留原始 Transcript 并追加具有完整来源的 Surface Replacement。Core Assistant Event 受 Step 生命周期约束，因此空闲时编辑角色历史使用明确封装的 `plugin/recall` user-role Node。
